...
import com.brent26.smith.mytodo_shoppingapp.DTO.ToDoItem
...
class DBHandler(val context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    override fun onCreate(db: SQLiteDatabase) {
        ...
        val createToDoItemTable =
                "CREATE TABLE $TABLE_TODO_LIST_ITEM (" +
                "$COL_ID integer PRIMARY KEY AUTOINCREMENT," +
                "$COL_CREATED_AT datetime DEFAULT CURRENT_TIMESTAMP," +
                "$COL_TODO_ID integer," +
    ...
    fun deleteToDoItem(itemId : Long){
        val db = writableDatabase
        db.delete(TABLE_TODO_LIST_ITEM, "$COL_ID=?", arrayOf(itemId.toString()))
    }
    ...
    fun addToDoItem(item : ToDoItem) : Boolean{
        ...
        val result = db.insert(TABLE_TODO_LIST_ITEM, null, cv)
        return result != (-1).toLong()
    }

    fun deleteToDo(todoId: Long){
        val db = writableDatabase
        db.delete(TABLE_TODO_LIST_ITEM, "$COL_TODO_ID=?", arrayOf(todoId.toString()))
        db.delete(TABLE_TODO, "$COL_ID=?", arrayOf(todoId.toString()))
    }

    fun updateToDoItemCompletedStatus(todoId: Long, isCompleted: Boolean){
        val db = writableDatabase
        val queryResult = db.rawQuery("SELECT * FROM $TABLE_TODO_LIST_ITEM WHERE $COL_TODO_ID=$todoId", null)
    ...
    fun updateToDoItem(item : ToDoItem){
        ...
        db.update(TABLE_TODO_LIST_ITEM, cv, "$COL_ID=?", arrayOf(item.id.toString()))
    }
